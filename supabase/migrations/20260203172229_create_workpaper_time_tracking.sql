/*
  # Create Workpaper Time Tracking System

  1. New Table
    - `time_logs`
      - Tracks time spent by auditors on workpapers
      - Links to workpapers and users
      - Supports daily activity logging with descriptions
      - Enables effort tracking and resource utilization analysis

  2. Schema Changes
    - Add `total_hours_spent` to workpapers table
      - Computed summary field for quick access
      - Updated via triggers automatically

  3. Security
    - Enable RLS on time_logs table
    - Users can view and manage their own time logs
    - Authenticated users can view team time logs

  4. Indexes
    - workpaper_id for fast lookups by workpaper
    - user_id for personal time tracking queries
    - activity_date for time-based filtering

  5. Functions
    - Auto-update total_hours_spent on workpapers
    - Calculate team effort metrics
*/
