...
// QA KONTROL LİSTESİ (Varsayılan Checklist)
...
interface Phase2Props {
  findingId: string;
  onNextPhase: () => void; // Onay -> Müzakereye git
  onRevision: () => void;  // Red -> Taslağa geri dön
}
...
export default function FindingStudioPhase2Page({ findingId, onNextPhase, onRevision }: Phase2Props) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex h-[calc(100vh-200px)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-100 p-8 flex justify-center border-r border-slate-200 custom-scrollbar">
          <div className="w-full max-w-[210mm] shadow-xl mb-12">
             <FindingPaper finding={finding} />
          </div>
      </div>

      {/* SAĞ PANEL: YÖNETİCİ KOKPİTİ */}
      <div className="w-[450px] bg-white flex flex-col border-l border-slate-200 shadow-xl z-10 shrink-0">
          
          {/* 1. YÖNETİCİ AKSİYONLARI */}
          <div className="p-6 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-2 mb-2">
                  <div className="px-3 py-1 bg-orange-100 text-orange-700 text-xs font-bold rounded-full border border-orange-200 flex items-center gap-1 w-fit">
              ...
              <div className="grid grid-cols-2 gap-3">
                  <button 
                      onClick={onRevision}
                      className="flex items-center justify-center gap-2 py-3 bg-white border border-red-200 text-red-600 rounded-xl font-bold text-sm hover:bg-red-50 transition-all shadow-sm active:scale-95"
                  >
                      <XCircle size={16}/> Revize İste
                  </button>
                  <button 
                      onClick={onNextPhase}
                      className="flex items-center justify-center gap-2 py-3 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200 active:scale-95"
                  >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
              ...
              <div>
                  ...
                  <div className="space-y-3 mb-4 max-h-48 overflow-y-auto custom-scrollbar p-1">
                      {finding.review_notes?.map(note => (
                          <div key={note.id} className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm animate-in fade-in slide-in-from-right-2">
                  ...
                  <div className="relative">
                      <textarea
                          ...
                          className="w-full p-3 pr-10 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none h-20"
                      />
                      <button 
