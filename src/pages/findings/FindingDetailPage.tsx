...
// BÖLÜM NAVİGASYON BUTONU
function NavButton({ id, label, icon: Icon, active, onClick }: any) {
    const isActive = active === id;
    return (
        <button 
            onClick={() => { onClick(id); document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }}
            className={clsx(
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden max-w-[1920px] mx-auto w-full">
        ...
        {/* SOL: BÖLÜM NAVİGASYONU */}
        <aside className="w-64 bg-white border-r border-slate-200 flex flex-col hidden lg:flex sticky top-0 h-[calc(100vh-64px)]">
           <div className="p-6">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider mb-4">Bulgu Bölümleri</h3>
              <nav className="space-y-1">
                 <NavButton id="tespit" label="1. Tespit & Kriter" icon={FileText} active={activeSection === 'tespit'} onClick={setActiveSection} />
                 <NavButton id="risk" label="2. Risk & Etki" icon={ShieldAlert} active={activeSection === 'risk'} onClick={setActiveSection} />
                 <NavButton id="koken" label="3. Kök Neden" icon={GitBranch} active={activeSection === 'koken'} onClick={setActiveSection} />
                 <NavButton id="oneri" label="4. Öneri & Aksiyon" icon={Target} active={activeSection === 'oneri'} onClick={setActiveSection} />
              </nav>
           </div>
           
           {/* Meta Bilgiler */}
           <div className="mt-auto p-6 border-t border-slate-100 bg-slate-50">
               <div className="text-xs text-slate-500 font-bold mb-2 flex items-center gap-1"><Building size={12}/> Denetlenen Birim</div>
               <div className="text-sm font-bold text-slate-800 truncate mb-3">{finding.auditee_department || 'Seçilmedi'}</div>
               
               <div className="text-xs text-slate-500 font-bold mb-2 flex items-center gap-1"><Calendar size={12}/> Tarih</div>
               <div className="text-sm font-bold text-slate-800 truncate">{new Date(finding.created_at).toLocaleDateString('tr-TR')}</div>
           </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto p-8 bg-slate-50 scroll-smooth">
           <div className="max-w-4xl mx-auto space-y-12 pb-24">
              ...
              {/* --- BÖLÜM 1: TESPİT --- */}
              <section id="tespit" className="scroll-mt-24">
                  <SectionHeader title="1. Tespit Detayları" icon={FileText} />
              ...
              <section id="risk" className="scroll-mt-24">
                  <SectionHeader title="2. Risk & Etki Analizi" icon={ShieldAlert} />
                  <div className="space-y-6">
              ...
              {/* --- BÖLÜM 3: KÖK NEDEN --- */}
              <section id="koken" className="scroll-mt-24">
                  <SectionHeader title="3. Kök Neden Analizi" icon={GitBranch} />
                  <div className="space-y-6">
                      <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-100 p-6 rounded-xl text-center">
              ...
              {/* --- BÖLÜM 4: ÖNERİ --- */}
              <section id="oneri" className="scroll-mt-24">
                  <SectionHeader title="4. Öneri & Aksiyon" icon={Target} />
                  <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                      <label className="block text-xs font-bold text-emerald-600 mb-3 uppercase flex items-center gap-2">
                          <CheckCircle2 size={14}/> Öneri ve Aksiyon Beklentisi
                      </label>
                      <RichTextEditor 
