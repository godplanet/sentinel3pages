...
import { 
  ...
  Upload, ExternalLink, AlertOctagon, Menu
} from 'lucide-react';
...
export default function FindingStudioPage() {
  ...
  // FAZ GEÇİŞ TETİKLEYİCİLERİ (State Update Simülasyonu)
  const handlePhaseChange = (nextTab: any, nextState: string) => {
      setActiveTab(nextTab);
      if(finding) setFinding({...finding, state: nextState as any});
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/30 flex flex-col font-sans">
        ...
        <div className="flex items-center gap-4">
            ...
            <div>
                <div className="flex items-center gap-2">
                    ...
                    {/* Constitution'dan renk alabiliriz ama şimdilik clsx ile devam */}
                    <span className={clsx("px-2 py-0.5 rounded text-[10px] font-bold border bg-slate-100")}>
                        {finding.severity}
                    </span>
                </div>
      ...
      <main className="flex-1 max-w-[1600px] mx-auto w-full p-8 pb-20">
        ...
        <div className="min-h-[500px]">
            {activeTab === 'overview' && <FindingStudioPhase1Page findingId={finding.id} onNextPhase={() => handlePhaseChange('review', 'IN_REVIEW')} />}
            {activeTab === 'review' && <FindingStudioPhase2Page findingId={finding.id} onNextPhase={() => handlePhaseChange('negotiation', 'NEGOTIATION')} onRevision={() => handlePhaseChange('overview', 'DRAFT')} />}
            {activeTab === 'negotiation' && <FindingStudioPhase3Page findingId={finding.id} onNextPhase={() => handlePhaseChange('final', 'FINAL')} />}
            {activeTab === 'final' && <FindingStudioPhase4Page findingId={finding.id} onNextPhase={() => handlePhaseChange('followup', 'CLOSED')} />}
