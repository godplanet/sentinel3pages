...
import { 
  CheckCircle2, XCircle, ShieldCheck, Lock, AlertOctagon, Save 
} from 'lucide-react';
...
interface Phase4Props {
  findingId: string;
  onNextPhase: () => void; // Master sayfadan gelen "Kapat" fonksiyonu
}

export default function FindingStudioPhase4Page({ findingId, onNextPhase }: Phase4Props) {
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          {actionPlans.map((plan) => (
              <GlassCard 
                  key={plan.id} 
                  className={clsx("p-6", plan.current_state === 'REJECTED' && "bg-red-50/30 border-red-200")}
                  neonGlow="none"
              >
                  ...
                  {rejectingId === plan.id && (
                      <div className="mt-4 p-4 bg-red-50 border border-red-100 rounded-xl animate-in fade-in slide-in-from-top-2">
                          ...
                          <textarea 
                              ...
                              className="w-full border border-red-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-red-500 mb-3 min-h-[80px] bg-white"
          ...
          <div className={clsx("transition-all duration-500", allPlansEvaluated ? "opacity-100" : "opacity-50 pointer-events-none grayscale")}>
              <GlassCard className="p-6">
                  <div className="mb-6 pb-6 border-b border-slate-100 text-center">
                      ...
                      <h2 className="text-xl font-bold text-slate-900">Dosya Kapanışı ve Başkan Onayı</h2>
                      <p className="text-sm text-slate-500 mt-1">
                          {!allPlansEvaluated 
                              ? "Kapanış için önce tüm aksiyon planlarını değerlendirip onaylamanız gerekmektedir." 
                              : "Tüm aksiyonlar onaylandı. Raporu kapatmak için imzalayınız."}
                      </p>
                  </div>
                  ...
                  <button 
                      onClick={onNextPhase}
                      className="w-full mt-6 py-4 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-95"
                  >
                      <Save size={18} /> Dosyayı Kapat ve Takibe Al
                  </button>
              </GlassCard>
          </div>
