...
import { fetchAuditorProfiles, updateAuditorProfile, type AuditorProfile } from '@/entities/talent';
...
export function SkillMatrix() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await fetchAuditorProfiles();
      setProfiles(data);
      if (data.length > 0) {
  ...
  const handleSaveCell = async () => {
    ...
    try {
      ...
      await updateAuditorProfile(profile.user_id, { skills_matrix: updatedSkills });
